...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        weight = nn.init.xavier_uniform_(w, gain=nn.init.calculate_gain('relu'))
        self.embed_to_hidden_weight = torch.nn.Parameter(weight)
        
        b = torch.empty(self.hidden_size)
        bias = nn.init.uniform_(b)
        self.embed_to_hidden_bias = torch.nn.Parameter(bias)
        
        self.dropout = nn.Dropout(p=dropout_prob)
        
        w = torch.empty(self.hidden_size, self.n_classes)
        weight = nn.init.xavier_uniform_(w, gain=nn.init.calculate_gain('relu'))
        self.hidden_to_logits_weight = torch.nn.Parameter(weight)
        
        b = torch.empty(self.n_classes)
        bias = nn.init.uniform_(b)
        self.hidden_to_logits_bias = torch.nn.Parameter(bias)
    ...
    def forward(self, w):
        ...
        hidden_state = relu( torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias )
        hidden_aft_d = self.dropout( hidden_state )
        logits =  torch.matmul(hidden_aft_d,self.hidden_to_logits_weight) + self.hidden_to_logits_bias
