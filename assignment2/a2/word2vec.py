...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat_matr = y_hat.reshape(y_hat.shape[0],1)
    centerWordVec_matr = centerWordVec.reshape(centerWordVec.shape[0],1)
    gradOutsideVecs = y_hat_matr.dot(centerWordVec_matr.T)
    gradOutsideVecs[outsideWordIdx] = centerWordVec*( y_hat[outsideWordIdx] - 1 )
...
def negSamplingLossAndGradient(
    ...
    loss = - np.log( sigmoid( outsideVectors[ indices[0] ].dot( centerWordVec ) ) ) - \
    	np.sum( np.log( ( sigmoid( - outsideVectors[ indices[1:] ].dot( centerWordVec ) ) ) ) )
    ...
    #gradOutsideVecs = gradOutsideVecs / 2
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:
    	...
    	output = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
    	loss += output[0]
    	gradCenterVecs[center_word_index] += output[1]
