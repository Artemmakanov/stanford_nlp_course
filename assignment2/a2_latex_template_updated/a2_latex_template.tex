...
\lstset{language=Python, % Use Perl in this example
        ...
        commentstyle=\u\vspace{-.7in}
sefont{T1}{pcr}{m}{sl}\color{MyDarkGreen}\small, % Comments small dark green courier font
...
$\bm y_w = \left \{0, 0, ... , 1, 0, ... ,0 \right \}$\\
$-\sum_{w \in \text{Vocab}} \bm y_w \log(\hat{\bm y}_w) = - [0*log(\hat{\bm y}_w_1) + 0*log(\hat{\bm y}_w_2) + ... + 1*log(\hat{\bm y}_w_{o}) + ... + 0*log(\hat{\bm y}_w_{|V|})] = 
- \log (\hat{\bm y}_o)$
...
\begin{shaded}
\begin{answer}
(1) The gradient equals zero for word $w$ when $ \hat{\bm y}_w = \bm y_w $
\newline
(2) There is because of optimization of function $\exp(\bm u_{o}^\top \bm v_c)$. The larger the scalar product, i.e., the greater the proximity of the vectors $\bm u_{o}^\top$ and $\bm v_c$, the smaller $J$.
\newline
$ \frac{\partial J}{\partial \bm v_c} = \bm U^T (\hat{\bm y} - \bm y). $
_
...
\begin{shaded}
\begin{answer} \\
$ \frac{\partial J}{\partial \bm u_w} = \bm v_c(\hat{\bm y}_w - \bm y_w).$	
...
$ \frac{\partial J}{\partial \bm U} = [\frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_1}, \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_2}, \cdots, \frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_{|\text{Vocab}|}}].$
...
\begin{enumerate}[label=(\roman*)]
\item $ \frac{\partial J_{neg-sample}}{\partial \bm v_c} = 
u_o(\sigma(\bm u_o^\top \bm v_c) -1) - \sum_{s=1}^K u_{w_{s}}(\sigma(-\bm u_{w_{s}}^\top \bm v_c ) - 1)
$. \newline
$ \frac{\partial J_{neg-sample}}{\partial \bm u_o} = 
v_c(\sigma(\bm u_o^\top \bm v_c) -1)$.
\newline
$ \frac{\partial J_{neg-sample}}{\partial \bm u_{w_{s}}} = 
v_c(1 - \sigma(-\bm u_{w_{s}}^\top \bm v_c))$.
\item 

\item Because with negative sample loss function it isn't neccesary to compute the denominator (the sum of exponential functions over the entire vocabulary), which is expensive for computation.
\end{enumerate}
...
$\frac{\partial J_{neg-sample}}{\partial \bm u_{w_{s}}} = 
\sum_{\substack{1\le k \le K \\ \\ u_{w_{s}} = u_{w_{k}}}} v_c(1 - \sigma(-\bm u_{w_{k}}^\top \bm v_c))
$
...
In this picture I can see that words, related to wheather was clustered together, 
also, there is a cluster of food: 'sweet', 'tea', 'coffee'.
There're a few gropus of adjectives: 'enfoyable', 'brilliant', 'cool'.
However, their antonyms are near: 'bad', 'annoying'.
Maybe, it's because they are related to one thing but from different ways.
Then, i can see analogy male:king :: woman:quenn.
_
\end{answer}
\end{shaded}
